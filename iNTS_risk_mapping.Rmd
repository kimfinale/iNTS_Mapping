...
#### POO by region
Aggregate the probabilityof occurrence by region (subnational region, country, and Africa subregion)
...
res = aggregate_by_region(rst=pred[[3]],
...
for (yr in years){
  ...
  poo_region[,as.character(yr)] = res$data$value
}

poo_region
data.table::fwrite(poo_region, paste0("outputs/poo_subregion_", tstamp(), ".csv"))

d=as.data.frame(t(round(as.matrix(poo_region[,as.character(2000:2020)]), digits=3)))
names(d) = poo_region$Area

data.table::fwrite(d, paste0("outputs/poo_subregion_long_", tstamp(), ".csv"), row.names=T)
