...
for (yr in years) {
  ...
  writeRaster(pred_calc, 
              filename=paste0("outputs/pred_",dist,"_calc_yr_", yr, "_", tstamp(hour=T), ".tif"), format="GTiff",
...
data.table::fwrite(poo_region, paste0("outputs/poo_subregion_",dist,"_", tstamp(), ".csv"))

d=as.data.frame(t(round(as.matrix(poo_region[,as.character(2000:2020)]), digits=3)))
names(d) = poo_region$Area

data.table::fwrite(d, paste0("outputs/poo_subregion_long_",dist,"_", tstamp(), ".csv"), row.names=T)
...
library(ggplot2)
dist = "dist3_100"
fits = list.files("outputs", "^fit_id", full.names=TRUE)
